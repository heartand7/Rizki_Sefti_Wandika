...
<head>
    ...
    <title>Edit Pengukuran Lahan - LahanKu</title>
...
<body class="bg-slate-900 text-slate-800 font-sans antialiased h-screen flex flex-col">
    ...
    <nav class="bg-white border-b border-slate-200 shadow-xs h-14 flex items-center justify-between px-6 shrink-0 z-50">
        <div class="flex items-center gap-3">
            ...
            <span class="text-md font-bold tracking-tight text-slate-900">Lahan<span class="text-emerald-600 font-medium">Editor</span></span>
        </div>
    ...
    <div class="flex flex-1 h-[calc(100vh-56px)] w-full overflow-hidden">
        ...
        <aside class="w-full md:w-[390px] lg:w-[430px] bg-white border-r border-slate-200 flex flex-col h-full shrink-0 z-40 shadow-xl">
            ...
            <div class="p-4 bg-slate-50 border-b border-slate-200 grid grid-cols-2 gap-3 shrink-0">
                <div onclick="openMetricModal()" class="bg-white p-3 rounded-xl border border-slate-200 hover:border-emerald-500 shadow-2xs metric-card cursor-pointer transition-all hover:bg-emerald-50/30 group">
                    <span class="block text-[10px] font-bold text-slate-400 group-hover:text-emerald-600 uppercase tracking-wider">Luas Terhitung <i class="fa-solid fa-expand text-[9px] ml-0.5 opacity-50"></i></span>
                    <div class="mt-0.5 flex items-baseline gap-1 overflow-hidden">
                        <span id="display-luas" class="text-lg md:text-xl font-mono font-bold text-slate-900 tracking-tight metric-value">{{ number_format($land->luas_meter, 2, ',', '.') }}</span>
                ...
                <div onclick="openMetricModal()" class="bg-white p-3 rounded-xl border border-slate-200 hover:border-emerald-500 shadow-2xs metric-card cursor-pointer transition-all hover:bg-emerald-50/30 group">
                    ...
                    <div class="mt-0.5 flex items-baseline gap-1 overflow-hidden">
                        <span id="display-keliling" class="text-lg md:text-xl font-mono font-bold text-slate-900 tracking-tight metric-value">{{ number_format($land->keliling_meter, 2, ',', '.') }}</span>
                        <span class="text-xs font-semibold text-slate-500 shrink-0">m</span>
                    </div>
                </div>
            </div>
            ...
            <div class="panel-scroll flex-1 p-4 space-y-4">
                <form action="{{ route('lands.update', $land->id) }}" method="POST" id="land-form">
                    @csrf
                    @method('PUT')
                    
                    <input type="hidden" name="luas_meter" id="input-luas" value="{{ $land->luas_meter }}">
                    <input type="hidden" name="keliling_meter" id="input-keliling" value="{{ $land->keliling_meter }}">
                    <input type="hidden" name="koordinat_polygon" id="input-koordinat" value='{!! json_encode($land->koordinat_polygon) !!}'>

                    <div class="space-y-3.5">
                        <div>
                            <label class="block text-xs font-bold text-slate-700 uppercase tracking-wide">Nama Pemilik Properti</label>
                            <input type="text" name="nama_pemilik" required value="{{ $land->nama_pemilik }}" class="mt-1 block w-full px-3 py-2 bg-white border border-slate-200 rounded-lg text-xs focus:outline-none">
                        </div>
                        <div>
                            <label class="block text-xs font-bold text-slate-700 uppercase tracking-wide">Nama / Label Identitas Lahan</label>
                            <input type="text" name="nama_lahan" required value="{{ $land->nama_lahan }}" class="mt-1 block w-full px-3 py-2 bg-white border border-slate-200 rounded-lg text-xs focus:outline-none">
                        </div>
                        <div>
                            <label class="block text-xs font-bold text-slate-700 uppercase tracking-wide">Lokasi / Alamat Lahan</label>
                            <div class="relative mt-1">
                                <input type="text" name="lokasi_alamat" id="input-alamat" required value="{{ $land->lokasi_alamat }}" class="block w-full pl-3 pr-10 py-2 bg-white border border-slate-200 rounded-lg text-xs focus:outline-none" placeholder="Alamat otomatis terisi setelah mengubah batas lahan...">
                        ...
                        <div>
                            <label class="block text-xs font-bold text-slate-700 uppercase tracking-wide">Deskripsi / Catatan Tambahan</label>
                            <textarea name="deskripsi" rows="3" class="mt-1 block w-full px-3 py-2 bg-white border border-slate-200 rounded-lg text-xs focus:outline-none">{{ $land->deskripsi }}</textarea>
                        </div>
                    </div>
        ...
        <main class="flex-1 relative h-full bg-slate-100">
            <div class="absolute top-4 floating-search-container space-y-1">
                <div class="flex items-center bg-white border border-slate-200 rounded-xl shadow-lg px-3 py-1.5 focus-within:border-emerald-500 transition-colors">
                    ...
                    <input type="text" id="gmaps-search-input" placeholder="Cari lokasi baru di Lahan ku..." class="w-full bg-transparent py-1 text-xs text-slate-800 outline-none font-medium">
        ...
        <div class="bg-white rounded-2xl border border-slate-200 shadow-2xl w-full max-w-sm overflow-hidden transform scale-95 transition-all duration-200">
            ...
            <div class="p-5 space-y-4">
                <div>
                    ...
                    <div class="text-base font-mono font-bold text-slate-900 mt-0.5"><span id="modal-luas-m2">0,00</span> <span class="text-xs font-sans font-medium text-slate-500">m²</span></div>
                </div>
                <div>
                    <span class="block text-[10px] font-bold text-slate-400 uppercase tracking-wider">Konversi Hektar</span>
                    <div class="text-base font-mono font-bold text-emerald-600 mt-0.5"><span id="modal-luas-ha">0,0000</span> <span class="text-xs font-sans font-medium text-slate-500">Ha</span></div>
                </div>
                <hr class="border-slate-100">
                <div>
                    <span class="block text-[10px] font-bold text-slate-400 uppercase tracking-wider">Keliling Bidang Standar</span>
                    <div class="text-base font-mono font-bold text-slate-900 mt-0.5"><span id="modal-keliling-m">0,00</span> <span class="text-xs font-sans font-medium text-slate-500">m</span></div>
                </div>
                <div>
                    <span class="block text-[10px] font-bold text-slate-400 uppercase tracking-wider">Konversi Kilometer</span>
                    <div class="text-base font-mono font-bold text-blue-600 mt-0.5"><span id="modal-keliling-km">0,0000</span> <span class="text-xs font-sans font-medium text-slate-500">Km</span></div>
                </div>
            </div>
    ...
    <script>
        let currentLuasRaw = {{ $land->luas_meter }};
        let currentKelilingRaw = {{ $land->keliling_meter }};
        ...
        window.addEventListener('load', function() {
            ...
            let currentPolygon = L.polygon(koordinatLeaflet, { color: '#10b981', weight: 3, fillOpacity: 0.2 }).addTo(drawnItems);
            ...
            // Nominatim Search (Pencarian Manual Atas)
            ...
            const drawControl = new L.Control.Draw({
                draw: false, edit: { featureGroup: drawnItems, remove: false }
            });
            map.addControl(drawControl);

            // REVERSE GEOCODING OTOMATIS SAAT POLYGON SELESAI DI-EDIT/DIGESER
            map.on(L.Draw.Event.EDITED, function (e) {
                e.layers.eachLayer(layer => {
                    const geojson = layer.toGeoJSON();
                    
                    // Rekalkulasi Metrik Baru
                    ...
                    // Cari titik tengah (Centroid) baru pasca poligon digeser
                    ...
                    alamatInput.value = "Memperbarui alamat titik lahan baru...";

                    // Ambil data lokasi geografis terbaru dari OpenStreetMap
                    fetch(`https://nominatim.openstreetmap.org/reverse?lat=${lat}&lon=${lng}&format=json&addressdetails=1`)
                        ...
                        .then(data => {
                            ...
                            if (data && data.display_name) {
                                ...
                                alamatInput.value = `Koordinat baru: ${lat.toFixed(6)}, ${lng.toFixed(6)}`;
                            }
                        })
                        .catch(err => {
                            loader.classList.add('hidden');
